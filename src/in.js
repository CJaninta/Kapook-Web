...
import $ from 'jquery';
import income from './components/img/in.png';
...
class kp extends Component {
    ...
    update(e, l, a) {
        ...
        if (this.state.newlist != " " || this.state.newam != " ") {
            if (this.state.newlist != l && this.state.newlist != " " && this.state.newlist != "-") {
                list_in = this.state.newlist;
            }
            if (this.state.newam != a && this.state.newam != " " && this.state.newam != "-") {
                amount_in = this.state.newam;
            }
            console.log(e, l, a);
            console.log(list_in, amount_in);
            $.ajax({
                type: 'POST',
                url: 'http://localhost/kapook-php/api/feedupin.php',
                data: { in_id: e, list: list_in, amount:  amount_in},
            });
        }
    ...
    onUpdate(e, l, a) {
        ...
        confirmAlert({
            ...
            message: <span className="Appfont" style={{ fontSize: "20px" }}><IoMdListBox style={{ fontSize: "27px", color: "rgb(0, 186, 80)" }} /> <span style={{ fontSize: "28px", color: "rgb(26, 112, 80)" }} >รายการ :</span> {l} <br />
                ...
                <span style={{ fontSize: "23px", color: "rgb(26, 112, 80)" }} >จำนวนเงินที่ได้รับ</span><Input onChange={this.onChange} type="number" name="newam" id="amount" placeholder="Enter your new amount" /><br />
    ...
    deleteFeedAction(e) {
        ...
        console.log(e);
        let data = JSON.parse(sessionStorage.getItem("userData"));
        let postData = { user_id: data.userData.User_ID, in_id: e };
        if (postData) {
            $.ajax({
                type: 'POST',
                url: 'http://localhost/kapook-php/api/feeddelin.php',
                data: { user_id: data.userData.User_ID,in_id:e},
            });
        }
        this.onre();
    }
    ...
    save(e) {
        ...
        e.preventDefault();
        let data = JSON.parse(sessionStorage.getItem("userData"));
        let postData = { user_id: data.userData.User_ID, list: this.state.topic, amount: this.state.income };

        console.log(postData, this.state.topic);
        if (this.state.topic) {
            $.ajax({
                type: 'POST',
                url: 'http://localhost/kapook-php/api/income.php',
                data: { user_id: data.userData.User_ID, list: this.state.topic, amount: this.state.income},
            });
        ...
        return (
            <React.Fragment>
                <div>
                        ...
                        <nav className="navbar sticky-top navbar-expand-lg navbar-dark bg-primary">
                            ...
                            <div className="collapse navbar-collapse nav" id="navbarNavAltMarkup">
                                <div className="navbar-nav ">
                                    ...
                                    <a className="nav-item nav-link active" href="/in">Income<span className="sr-only">(current)</span></a>
                    ...
                    <div className="inf">
                        <center><h1 style={{ fontSize: "70px", fontWeight: "700" }}><img style={{ height: "10vmin" }} src={income} className="App-logo" /> รายรับ</h1></center>
                    </div>
                    <div className="log inf">
                        <button type="button" class="btn btn-outline-success" style={{ float: "right" ,border:"4px solid",fontSize:"20px",fontWeight:"500"}} data-toggle="collapse" href="#multiCollapseExample1" role="button" aria-expanded="false" aria-controls="multiCollapseExample1">เพิ่มรายการ</button>
                    </div>
                    <div className="paper">
                        <div className="boxin">
                            <div class="col inf">
                                    ...
                                    <form onSubmit={this.save} method="post" id="input">
                                        ...
                                        <div className="input">
                                            <div class="input-group mb-3">
                                                ...
                                                <input value={this.state.income} onChange={this.onChange} name="income" type="text" placeholder="จำนวนเงินที่ได้รับ" class="form-control" aria-label="Amount (to the nearest dollar)" />
                                        ...
                                        <div className="input1">
                                            <div class="input-group mb-3">
                                                <button value="Post" onClick={this.save} type="submit" class="btn btn-success" style={{ width: "100%", borderRadius: "20px" }}>ตกลง</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div className="boxinfo">
                            {data.map(data => (
                                <div>
                                    ...
                                    <div className="income card text-white bg-success mb-3" >

                                        <span style={{ color: "rgb(144, 255, 0)" }}><IoMdListBox style={{ fontSize: "27px" }} /> รายการ : {' '}
                                            <span style={{ fontSize: "25px", color: "white" }}>{data.List_name}</span>

                                        </span>
                                        <span style={{ color: "rgb(144, 255, 0)" }}><IoIosCash style={{ fontSize: "27px" }} /> จำนวน :  {' '}
                                            <span style={{ fontSize: "25px", color: "white" }}>{data.Amount} บาท</span>
                                        </span>
                                        <span style={{ color: "rgb(115, 255, 0)", paddingRight: "20px" }}>
                                            <span style={{ fontSize: "18px", color: "white", float: "right" }}>{data.Date}</span>
                                        </span>

                                    </div>
                                    {/* เลขไอดีของแต่ละรายการ*/}<span style={{ fontSize: "30px", float: "right" }}>{/*data.In_ID*/}</span>

                                    <span style={{ float: "right", paddingTop: "10px" }}>
                                        <button id="del" class="btn btn-danger" onClick={(e) => this.deleteFeed(data.In_ID, data.List_name, data.Amount)} data={data.In_ID} style={{ padding: "1px", margin: "3px" }}>
                                            <MdClear style={{ fontSize: "30px", color: "white" }} />
                                        </button>
                                        <br />
                                        <button id="up" class="btn btn-info" onClick={(e) => this.onUpdate(data.In_ID, data.List_name, data.Amount)} data={data.In_ID} style={{ padding: "1px", margin: "3px" }}>
